...
public class UserController {
    ...
    public ResponseEntity<UserDTO> registerUser(@Valid @RequestBody RegisterRequest request, HttpServletRequest httpRequest) {
        log.info("Registration request received for username: {}, email: {}", request.getUsername(), request.getEmail());
        
        // Register user in Keycloak
        String keycloakId;
        try {
            keycloakId = keycloakService.registerUser(
                request.getUsername(),
                request.getEmail(),
                request.getPassword(),
                request.getRole()
            ...
            log.error("Failed to register user in Keycloak: {}", e.getMessage(), e);
        ...
        try {
            ...
            throw new IllegalArgumentException("Invalid role: " + request.getRole() + ". Valid roles are: USER, FREELANCER, ENTERPRISE, ADMIN");
        }
