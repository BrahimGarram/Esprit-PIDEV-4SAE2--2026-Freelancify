...
public class MessageController {
     ...
     * Participants (client + assigned freelancer) see only their conversation.
     * ADMIN sees the full conversation (watch/join).
     */
    ...
    public ResponseEntity<List<MessageDTO>> getProjectMessages(
        ...
        if (currentUser.getRole() == UserRole.ADMIN) {
            messages = messageService.getMessagesByProjectAsAdmin(projectId);
        } else {
            messages = messageService.getMessagesByProject(projectId, currentUser.getId());
        }
