...
public interface MessageRepository extends JpaRepository<Message, Long> {
    ...
    /**
     * Find messages for a project room (current user must be sender or receiver)
     */
    @Query("SELECT m FROM Message m WHERE m.projectId = :projectId AND (m.sender.id = :userId OR m.receiver.id = :userId) ORDER BY m.sentAt ASC")
    List<Message> findByProjectIdAndParticipantOrderBySentAtAsc(@Param("projectId") Long projectId, @Param("userId") Long userId);

    /**
     * Find all messages for a project (admin only - no participant filter)
     */
