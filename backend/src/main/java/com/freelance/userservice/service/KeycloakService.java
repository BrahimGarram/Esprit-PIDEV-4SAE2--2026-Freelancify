...
public class KeycloakService {
    ...
    private String getAdminAccessToken() {
        try {
            ...
            log.info("Requesting Keycloak admin token from {}", tokenUrl);
            ...
            log.error(
                "Keycloak admin token request failed. url={}, status={}, body={}",
    ...
    public String registerUser(String username, String email, String password, String role) {
        try {
            ...
            // Assign realm role if it exists in Keycloak (skip if role not created in realm)
            ...
            try {
                ...
                if (e.getStatusCode().value() == 404) {
                    log.warn("Realm role '{}' not found in Keycloak realm '{}'. User {} created but without this role. Create the role in Keycloak Admin Console (Realm roles) if needed.", role, realm, username);
                } else {
